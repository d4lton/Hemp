...
/*
          ...
          var coordinates = this._windowToCanvas(event.offsetX, event.offsetY);
          var objects = this._findObjectsAt(coordinates.x, coordinates.y);
          this._deselectAllObjects();
          if (objects.length > 0) {
            var selectedObject = objects[objects.length - 1];
            this._selectObject(selectedObject);
            this._element.dispatchEvent(new CustomEvent('select', {'detail': selectedObject}));
          } else {
            this._element.dispatchEvent(new CustomEvent('select', {'detail': null}));
          }
*/
        }
      };
